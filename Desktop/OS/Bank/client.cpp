...
void process_command(Bank* bank, const std::string& cmd) {
    ...
    if(action == "max_balance") {
        int account;
        iss >> account;

        if(!is_valid_account(bank, account)) {
            std::cout << "Sory account " << account << "doesn`t exist\n";
        }
        else {
        ...
        std::cout << "\nAvailable commands:\n"
                  ...
                  << "  withdraw_all ry<amount>     - withdraw from all accounts\n"
                  << "  help                      - show this help\n"
...
int main()
{
    ...
    if (fd == -1) {
        std::cout << "Sorry, couldn't connect to the banking system. The bank may not be initialized\n";
        return 1;
    }

    Bank* bank = static_cast<Bank*>(mmap(nullptr, sizeof(Bank), PROT_READ, MAP_SHARED, fd, 0));
    if (bank == MAP_FAILED) {
        std::cout << "Sorry, an internal system error occurred\n";
        close(fd);
        return 1;
    }

    size_t size = sizeof(Bank) + (bank->size - 1) * sizeof(BankCell);
    munmap(bank, sizeof(Bank));
